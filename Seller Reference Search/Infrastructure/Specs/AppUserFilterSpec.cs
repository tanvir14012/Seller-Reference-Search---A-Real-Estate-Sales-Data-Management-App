...
{
    public class AppUserFilterSpec: Specification<AppUser>
    {
        public AppUserFilterSpec(DtParameters<AppUser> dtParameters)
        {
        ...
        private void ApplySearch(string searchValue, DtColumn[] columns)
        {
            ...
            if (orExpression != null)
            {
                var lambda = Expression.Lambda<Func<AppUser, bool>>(orExpression, parameter);
                Query.Where(lambda);
            }
